...
// 3. Controller Utama Chat (Teks, File, & Hybrid Image Generation/Editing)
exports.handleGeneralChat = async (req, res) => {
    try {
        ...
        // =========================================================================
        // [FITUR BARU] INTERSEPTOR REQUEST GAMBAR (TEXT-TO-IMAGE & IMAGE EDITING)
        // =========================================================================
        const isImageRequest = /gambar|foto|lukis|bikin|buatkan|edit|ubah|ganti|manipulasi/i.test(message);

        if (isImageRequest) {
            let finalPrompt = message;

            // KASUS KHUSUS: User mengirim file dan file tersebut adalah gambar/foto
            if (fileData && fileMimeType && fileMimeType.includes('image/')) {
                console.log("📸 User mengirim gambar beserta instruksi modifikasi/buat ulang.");

                // Meminta bantuan Gemini Vision untuk menganalisis gambar + perintah user, lalu membuat prompt bahasa Inggris yang matang
                const filePart = fileToGenerativePart(fileData, fileMimeType);
                const systemInstruction = { 
                    text: `User mengirimkan sebuah foto/gambar dan memberikan instruksi: "${message}". 
                    Tugasmu adalah membuat deskripsi atau prompt gambar baru berbahasa Inggris (English) yang sangat detail berdasarkan foto tersebut sesuai dengan request edit atau modifikasi dari user agar bisa digambar ulang oleh AI generator.
                    CONTOH: Jika user mengirim foto orang dan menulis "ganti baju jadi jas hitam", kamu mendeskripsikan penampilan orang tersebut namun dengan pakaian jas hitam lengkap secara profesional dan detail.
                    JANGAN berikan teks obrolan atau basa-basi apa pun, LANGSUNG berikan hasil prompt bahasa Inggrisnya saja!` 
                };

                const visionResult = await aiModel.generateContent([filePart, systemInstruction]);
                finalPrompt = visionResult.response.text().trim();
                
                console.log(`🤖 Prompt hasil terjemahan Gemini Vision: "${finalPrompt}"`);
            } else {
                console.log(`🎨 User meminta gambar murni dari teks via Hugging Face. Prompt: "${message}"`);
            }

            // Memanggil model pembuat gambar (Stable Diffusion XL) gratisan menggunakan prompt akhir
            const imageBlob = await hf.textToImage({
                model: 'stabilityai/stable-diffusion-xl-base-1.0',
                inputs: finalPrompt,
                parameters: {
                    negative_prompt: 'blurry, bad quality, distorted, extra limbs, low resolution, deformed face, bad anatomy',
                }
            });
