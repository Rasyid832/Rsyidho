...
// [BARU] Variabel global untuk mengontrol pembatalan chat
...
function parseAndRenderMath(text) {
    ...
    // 3. Render Format Tiga Bintang (Bold + Italic) ***teks*** -> <strong><em>teks</em></strong>
...
async function sendChatMessage() {
    ...
    // [BARU] Jika AI sedang aktif merespons, klik pada tombol ini akan memicu pembatalan (STOP)
    if (chatAbortController) {
        chatAbortController.abort(); // Putuskan proses request/typing
        return;
    }
    ...
    // [BARU] Inisialisasi AbortController sebelum menembak request fetch ke backend
    chatAbortController = new AbortController();
    
    // [BARU] Ubah visual tombol kirim bawaan menjadi tombol "Stop" merah jingga
    const sendBtn = document.getElementById('chat-send-button'); 
    if (sendBtn) {
        sendBtn.innerHTML = '⏹️ Stop';
        sendBtn.className = "bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs font-medium px-4 py-3 rounded-xl shadow-md transition-all shrink-0 flex items-center gap-1";
    }

    try {
        // Tembak paket data lengkap ke backend dengan menyisipkan signal abort
        const response = await fetch(`${BACKEND_URL}/chat`, {
            ...
            signal: chatAbortController.signal // [BARU] Pasang sinyal abort disini
        });
        ...
        if (data.error) {
            ...
            // [BARU] Menerima sinyal pembatalan apabila user menekan Stop di tengah-tengah efek mengetik berjalan
            chatAbortController.signal.addEventListener('abort', () => {
                isAbortedDuringTyping = true;
            });
            
            function typeWriterEffect() {
                // [BARU] Evaluasi status abort setiap siklus cetak karakter berjalan
                if (isAbortedDuringTyping) {
                    targetTargetBubble.insertAdjacentHTML('beforeend', ' <span class="text-orange-400 text-xs font-semibold block mt-2">⏹️ Generation stopped by user.</span>');
                    chatContainer.scrollTop = chatContainer.scrollHeight;
                    resetSendButton();
                    return;
                }
                ...
                if (currentTextIndex <= rawContent.length) {
                    ...
                    targetTargetBubble.innerHTML = processedHtml;
                    chatContainer.scrollTop = chatContainer.scrollHeight;
                    resetSendButton(); // Balikkan tombol menjadi ungu normal saat ketikan usai
                }
            }
        ...
        // [BARU] Tangani respons jika pembatalan dipicu saat data sedang dimuat dari backend (Loading state)
...
// [BARU] Fungsi pembantu untuk mengembalikan wujud asli tombol "Send" ungu milikmu
function resetSendButton() {
    chatAbortController = null;
    const sendBtn = document.getElementById('chat-send-button');
    if (sendBtn) {
        sendBtn.innerHTML = 'Send';
        // Mengembalikan styling kelas Tailwind ungu gradasi asli milikmu
        sendBtn.className = "bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-sm font-medium px-5 py-3 rounded-xl shadow-md hover:shadow-lg transition-all shrink-0 flex items-center justify-center";
    }
}