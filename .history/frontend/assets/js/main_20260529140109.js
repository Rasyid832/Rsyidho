...
function parseAndRenderMath(text) {
    ...
    // 1. Render Format Markdown Tebal (Bold) **teks** -> <strong>teks</strong>
    text = text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');

    // [BARU] Render Format Markdown Miring (Italic) *teks* -> <em>teks</em>
    text = text.replace(/\*(.*?)\*/g, '<em>$1</em>');

    // 2. Handle Display Math ($$.$$) terlebih dahulu
    ...
    // 3. Handle Inline Math ($.$) yang suka menempel ke huruf/tanda baca lain
