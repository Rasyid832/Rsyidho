...
function loadChatMessages() {
    ...
    currentRoom.messages.forEach(msg => {
        if (msg.sender === 'user') {
            ...
            // Re-highlight dengan Prism jika data baru dimuat
...
function parseAndRenderMath(text) {
    ...
    // 1. Handle Code Blocks (```nama_bahasa ... ```)
    text = text.replace(/```([a-zA-Z0-9+#-]+)?\n([\s\S]*?)```/g, (match, lang, code) => {
        const language = lang ? lang.trim().toLowerCase() : 'plaintext';
        const cleanCode = code.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        
        return `
            <div class="my-4 rounded-xl overflow-hidden border border-slate-800 bg-slate-950 shadow-lg text-left font-mono text-xs sm:text-sm">
                <div class="bg-slate-900 px-4 py-1.5 flex justify-between items-center border-b border-slate-800 text-slate-400 select-none">
                    ...
                    <button onclick="navigator.clipboard.writeText(\`${code.trim().replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`); this.innerText='Copied!'; setTimeout(() => this.innerText='Copy', 2000)" class="text-[11px] hover:text-white transition cursor-pointer font-sans font-medium">
                        Copy
                    </button>
                </div>
        ...
        if (isResponseImage) {
            ...
            function typeWriterEffect() {
                ...
                if (currentTextIndex <= rawContent.length) {
                    ...
                    // Highlight secara real-time selagi efek mengetik berjalan
                    ...
                    // Pastikan hasil akhir ter-highlight sempurna setelah selesai mengetik
                    if (window.Prism) Prism.highlightAllUnder(targetBubble);
