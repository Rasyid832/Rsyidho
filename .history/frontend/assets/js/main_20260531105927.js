...
// Fungsi Merender Daftar Riwayat Chat di Sidebar Kiri (Fixed Layout Bug)
function renderRecentChats() {
    ...
    chatRooms.forEach(room => {
        ...
        const lastMessage = room.messages[room.messages.length - 1]?.text || 'Tidak ada pesan';
        
        // Membersihkan simbol kode Markdown agar preview teks di sidebar rapi
        const cleanPreview = lastMessage.replace(/[`#*]/g, '').trim();
        ...
        const roomHtml = `
            <div onclick="switchChatRoom('${room.id}')" class="group relative p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition cursor-pointer flex items-center justify-between gap-2 ${activeClasses}">
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-[10px] sm:text-xs text-slate-400 mt-0.5 truncate block max-w-full">${cleanPreview}</p>
                </div>
...
function parseAndRenderMath(text) {
    ...
    text = text.replace(/\*\*\trim(.*?) \*\*\*/g, '<strong><em>$1</em></strong>');
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (isResponseImage) {
            ...
            // DETEKSI UTAMA: Jika AI mengembalikan Code Block (```), bypass typewriter agar HTML tidak rusak
            const containsCodeBlock = rawContent.includes('```');
            ...
            if (containsCodeBlock) {
                ...
                // Jika hanya obrolan teks biasa tanpa kode, jalankan typewriter seperti semula
